...
- [Проект по курсу "Проектирование высоконагруженных систем".](#проект-по-курсу-проектирование-высоконагруженных-систем)
  ...
  - [3. Глобальная балансировка нагрузки](#3-глобальная-балансировка-нагрузки)
    - [Cхема глобальной балансировки до датацентров](#cхема-глобальной-балансировки-до-датацентров)
    - [Физическое расположение датацентров](#физическое-расположение-датацентров)
...
|  Проведение трансляции   |    399 Гбит/с    |    557 Гбит/с    | 4 250 000 312 ГБайт/сутки |
| Отправка сообщений в чат |   0.002 Гбит/с   |  0.0034 Гбит/с   |    12 312 ГБайт/сутки     |
...
Количество запросов на поиск достаточно сложно подсчитать, т.к. в откытых источниках не достаточно для этого информации. Допусти, что 30% всех пользователей пользуются поиском и делают в среднем 3 поисковых запроса в день(остальные приходят только на трансляции по уведомлениям).
...
| Тип запроса         | RPS  |
| :------------------ | :--- |
| Открытие трансляции | 1023 |
| Отправка сообщения  | 600  |
| Поиск               | 323  |
| Cтарт трансляции    | 15   |

## 3. Глобальная балансировка нагрузки

### Cхема глобальной балансировки до датацентров

### Физическое расположение датацентров

Исходя из географии аудитории представленной в пункте 1, можно сделать вывод, что большая часть аудитории приходится на Северную Америку(39%), поэтому необходимо там разместить достаточное количество ДЦ. Датацентры будут распологаться в США, прежде всего в крупных городах. Поэтому ДЦ будут находиться в следующих локациях:
...
Так же необходимо расположить ДЦ в Канаде, пусть это будет город Торонто.

На Европу приходится 32% процента пользователей, причем пользователи равномерно распределены по всей территории. Поэтому распределение будет следующим:
...
В азии платформа не пользуется популярностью в Китае в виду политики страны, там есть свои аналоги, поэтому большая часит азиатской аудитории приходится на Японию и Южную Корею, так же стоит учитвать аудиторию с азиатской части России. Поэтому имеем следующее расположение:
...
Все размещенные датацентры покрывают основную часть трафика, осталось не затронутыми:

- Австралия
- Южная Америка

На Бразилию и Аргентину приходится 10% пользователей поэтому располагаем ДЦ в следующих местах:

- Рио-де-Жанейро
- Бразилиа
- Буэнос-Айрос

В Австралии располагается всего 1.6% пользователей, а так же с учетом плотности населения можно расположить всего один ДЦ в столице, т.е. в городе Канберра

### Схема примерного расположения ДЦ

![Схема расположения ДЦ](./assets/dc.png)
